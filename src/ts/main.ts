...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function initMainPageLayout() {
    const switchToVerticalBtn = document.getElementById(
      "switch-to-vertical"
    ) as HTMLButtonElement;
    const switchToHorizontalBtn = document.getElementById(
      "switch-to-horizontal"
    ) as HTMLButtonElement;
    const verticalLayout = document.getElementById(
      "layout-vertical-scroll"
    ) as HTMLElement;
    const horizontalLayout = document.getElementById(
      "layout-horizontal-carousel"
    ) as HTMLElement;
    const parallaxLayer = document.querySelector(
      "#layout-horizontal-carousel"
    ) as HTMLElement;
    const layoutParam = new URLSearchParams(window.location.search).get(
      "layout"
    );
  ...
  function initVerticalScrollAnimations() {
    ...
    heroTimeline
      .from("#layout-vertical-scroll .hero h1", {
        opacity: 0,
        y: -50,
        duration: 1,
        ease: "power2.out",
    ...
    animals.forEach((animal) => {
      ...
      gsap.from([h2, a], {
        ...
        stagger: 0.2,
        ease: "power2.out",
        scrollTrigger: { trigger: sectionId, start: "top 75%" },
      });
      ...
      switch (animal) {
        ...
        case "rhino":
          ...
          break;
        case "elephant":
          const elephantChars = originalText.split("");
          p.innerHTML = elephantChars
            .map(
              (char) =>
  ...
  function initHorizontalCarouselAnimations(parallaxLayer: HTMLElement) {
    const slides = gsap.utils.toArray(
      "#layout-horizontal-carousel .carousel-slide"
    ) as HTMLElement[];
